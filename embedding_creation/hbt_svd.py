...
def run_mse_w2v(
    ...
    # Make the embedder
    embedder, solver = h.embedder.get_mse_w2v_embedder(
        bigram=bigram,
...
def describe(embedder, solver):
    ...
    description += "\n> f_delta\n"
    description += "k\t{}\n".format(f_delta.k)
    description += "clip_after_shift\t{}\n".format(f_delta.clip_after_shift)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--clip_after_shift', '-c', action="store_true",
        help=(
            "Clip negative values after shifting by -lg(k).  "
            "Default is to clip before shifting."
        )
    )
