...
def run_glv(
    ...
    embedder, solver = h.embedder.get_glv_embedder(
        bigram=bigram, 
        d=d,
        init_vecs=init_vecs,
        X_max=X_max,
...
def describe(embedder, solver):
    ...
    description += "\n> f_delta\n"
    description += "X_max\t{}\n".format(f_delta.X_max)
    description += "alpha\t{}\n".format(f_delta.alpha)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--X-max', '-X', type=float, default=100, 
        help="Loss function threshold hyperparameter."
    )
    parser.add_argument(
